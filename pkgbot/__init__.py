...
def process_request(data):
    ...
    if isinstance(repo_conf, dict):
        try:
            ...
            logger.error(
                "{0} [dict-type] - Config for repo not found or invalid Error: {1}".format(repo, e)
            )
            return
    ...
    # .pkg-bot.yml is list-type, find actual block
        ...
        if not block_match:
            logger.error(
                "{0} - No block in .pkg-bot.yml matches to branch "
                "'{1}'".format(repo, data["ref"])
            )
            return

        if has_duplicate:
            logger.error(
                "{0} - Multiple matches for branch: {1} found".format(repo, duplicate_branch_name)
            )
            return
    ...
    if not has_sock:
        logger.error(
            "{0} - Cannot connect to aptly-spooler, socket: "
    ...
    if not os.path.isdir(incoming_pkg_dir):
        logger.error("{0} - Directory for repo '{1}' not found".format(repo, incoming_pkg_dir))
        return

    # branch is also required in config
    if data["ref"] not in valid_branches:
        logger.info("{0} - Branch: {1} does not match any configured".format(repo, data["ref"]))
        return
    ...
    if "stages" in repo_conf:
        if data["build_stage"] not in repo_conf["stages"]:
            logger.info(
                "{0} - Build stage {1} does not match any "
                "configured".format(repo, data["build_stage"])
            )
            return
    ...
    if "download-delay" in repo_conf:
        logger.info(
            "{0} - Config has delay in it, sleep for {1} secs".format(
                repo, repo_conf["download-delay"]
            )
        )
    ...
    logger.info("{0} - Downloaded to: {1}".format(repo, dl_path))
    ...
    for distro in pkg_data:
        ...
        for version in pkg_data[distro]:
            ...
            for item in glob_list:
                ...
                if len(glob_match) > 1:
                    logger.error(
                        "{0} - Multiple matches on distro: {1} "
                        "version: {2}  glob: '{3}'".format(repo, distro, version, item)
                    )
                    error_count += 1
            ...
            if not pkgs_match[distro][version]:
                logger.warning(
                    "{0} - No matches for distro: {1} version: "
                    "{2}".format(
                        repo,
                        distro,
                        version,
    ...
    if error_count > 0:
        logger.error("{0} - Found {1} errors, will not continue".format(repo, error_count))
        ...
        for version in pkgs_match[distro]:
            ...
            for pkg in pkgs_match[distro][version]:
                ...
                if os.path.isfile(pkg_fullpath):
                    logger.warning(
                        "{0} - File {1} already exists, " "skipping".format(repo, pkg_file)
                    )
                    continue

                logger.info(
                    "{0} - ADDPKG - distro: {1} version: {2} package: "
                    "{3}".format(repo, distro, version, pkg)
                )
                ...
                if distro in ["rhel", "centos", "fedora", "suse"]:
                    has_rpm = True
    ...
    if has_rpm:
        ...
        for pkg_fullpath, repo, distro, version in rpm_add:
            ...
            try:
                ...
                logger.error(
                    "{0} - Error copying file: {1} to {2} ".format(repo, pkg_fullpath, copy_to)
                )
                continue

            # sign rpm
            rpmsign_cmd = [conf["pkgbot"]["scripts"]["rpm-sign"], copy_to]
            subprocess.check_call(rpmsign_cmd)
            createrepo_distros.append(conf["pkgbot"]["package-structure"][distro][version])
        ...
        for distro in createrepo_distros:
            ...
            sign_repodata_cmd = [
                ...
                "--detach-sign",
                "--armor",
                "{0}/repodata/repomd.xml".format(rpm_repo_path),
            ]
...
def main():
    ...
    try:
        ...
        logger.info("Started {0} server on port {1}".format(__version__, conf["pkgbot"]["port"]))
